      ...
      'select': {
        ...
        'eventexprs': {
          'expr_0': function (ctx) {
            ...
            if (m.length > 1)
              matches.push(m[1]);
            ctx.rule.scope.set('my_name', matches[0]);
            return true;
          }
        },
        'state_machine': {
      ...
      'action_block': {
        'actions': [function (ctx) {
            return {
              'type': 'directive',
              'name': 'store_name',
              'options': { 'name': ctx.rule.scope.get('my_name') }
            };
          }]
      },
      'postlude': {
        ...
        'notfired': undefined,
        'always': function (ctx) {
          ctx.db.putEntVarFuture(ctx.pico.id, 'name', ctx.rule.scope.get('my_name')).wait();
        }
      }
    },
    'store_appvar': {
      ...
      'action_block': {
        'actions': [function (ctx) {
            return {
              'type': 'directive',
              'name': 'store_appvar',
              'options': { 'appvar': ctx.rule.scope.get('my_name') }
            };
          }]
      },
      'postlude': {
        ...
        'notfired': undefined,
        'always': function (ctx) {
          ctx.db.putAppVarFuture(ctx.rid, 'appvar', ctx.rule.scope.get('my_name')).wait();
        }
      }
    }
  }
