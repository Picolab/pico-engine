...
module.exports = function(ctx, rulesets, callback){
  if(!_.has(rulesets, ctx.meta.rid)){
    return callback(new Error('Not found: rid'));
  }
  if(!_.has(rulesets[ctx.meta.rid].provided_query_fns, ctx.fn_name)){
    return callback(new Error('Not found: function'));
  }
  var fn = rulesets[ctx.meta.rid].provided_query_fns[ctx.fn_name];
  if(!_.isFunction(fn)){
