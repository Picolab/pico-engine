ruleset io.picolabs.event-exp {
    ...
    rule or_duppath {
        select when (ee_or_duppath a before ee_or_duppath a) or ee_or_duppath a

        send_directive("(a before a) or a");
    }
    rule notbet_duppath {
        select when ee_notbet_duppath a not between (ee_notbet_duppath b, ee_notbet_duppath a)
    ...
    rule ab_or_ca {
        select when (ee_ab_or_ca a and ee_ab_or_ca b)
                 or (ee_ab_or_ca c and ee_ab_or_ca a)
