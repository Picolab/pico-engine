module.exports = {
  ...
  "global": function* (ctx) {
    ctx.scope.set("doubleThis", ctx.KRLClosure(ctx, function* (ctx, getArg) {
    ...
    "scope": {
      ...
      "foreach": function* (ctx, foreach, iter) {
        yield foreach(yield ctx.scope.get("doubleThis")(ctx, [[
          ...
          yield foreach(ctx.scope.get("arr"), ctx.KRLClosure(ctx, function* (ctx, getArg) {
            ctx.scope.set("foo", getArg("value", 0));
            yield foreach(yield ctx.callKRLstdlib("range", 0, ctx.scope.get("foo")), ctx.KRLClosure(ctx, function* (ctx, getArg) {
