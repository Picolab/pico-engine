module.exports = {
  ...
  "init": async function ($rsCtx, $mkCtx) {
    ...
    const wellKnown_Rx2 = $ctx.krl.Function([], async function () {
      ...
      return await head1($ctx, [await $ctx.krl.assertFunction($ctx.module("wrangler")["channels"])($ctx, [tags3])]);
    });
    ...
    $rs.when($ctx.krl.SelectWhen.e("wrangler:ruleset_installed", async function ($event, $state) {
      ...
      try {
        $ctx.setCurrentRuleName("initialize");
        $ctx.log.debug("rule selected", { "rule_name": "initialize" });
        ...
        if ($fired) {
          await $ctx.rsCtx.raiseEvent("wrangler", "need_wellKnown_Rx", $ctx.module("event")["attrs"]($ctx));
        }
      } finally {
        $ctx.setCurrentRuleName(null);
      }
    });
    $rs.when($ctx.krl.SelectWhen.e("wrangler:need_wellKnown_Rx"), async function ($event, $state, $last) {
      try {
    ...
    $rs.when($ctx.krl.SelectWhen.e("engine:started"), async function ($event, $state, $last) {
      try {
        $ctx.setCurrentRuleName("create_root_pico_wellKnown_Rx");
        $ctx.log.debug("rule selected", { "rule_name": "create_root_pico_wellKnown_Rx" });
        const channel3 = await wellKnown_Rx2($ctx, []);
        var $fired = await isnull1($ctx, [channel3]);
        if ($fired) {
          await noop1($ctx, []);
    ...
    $rs.when($ctx.krl.SelectWhen.e("wrangler:established_removal"), async function ($event, $state, $last) {
      try {
        ...
        if ($fired) {
          await $ctx.krl.assertAction($ctx.module("wrangler")["deleteChannel"])($ctx, [await $stdlib["get"]($ctx, [
              bus3,
              "Rx"
            ])]);
        }
    ...
    $rs.when($ctx.krl.SelectWhen.e("wrangler:intent_to_delete", async function ($event, $state) {
      var matches = [];
      var setting = {};
      var m;
      var j;
      m = new RegExp("^(.+)$", "").exec(Object.prototype.hasOwnProperty.call($event.data.attrs, "Id") ? $stdlib.as($ctx, [
        $event.data.attrs["Id"],
        "String"
      ]) : "");
      if (!m)
      ...
      try {
        $ctx.setCurrentRuleName("prepare_to_delete");
        $ctx.log.debug("rule selected", { "rule_name": "prepare_to_delete" });
        var Id3 = $state.setting["Id"];
        this.rule.state = Object.assign({}, $state, { "setting": {} });
        const the_subs3 = await established2($ctx, [
          "Id",
          Id3
        ]);
        var $fired = the_subs3;
        ...
        if ($fired)
          ...
          $ctx.log.debug("not fired");
        if ($fired) {
          await $ctx.rsCtx.raiseEvent("wrangler", "subscription_cancellation", $ctx.module("event")["attrs"]($ctx));
          await $ctx.rsCtx.raiseEvent("wrangler", "ready_for_deletion", undefined);
        }
      } finally {
        $ctx.setCurrentRuleName(null);
      }
    ...
    return {
      ...
      "query": {
        "established": async function (query, qid) {
          $ctx.setQuery(Object.assign({}, query, { "qid": qid }));
          try {
            return await established2($ctx, query.args);
          } finally {
            $ctx.setQuery(null);
          }
        },
        "outbound": async function (query, qid) {
          $ctx.setQuery(Object.assign({}, query, { "qid": qid }));
          try {
            return await outbound2($ctx, query.args);
          } finally {
            $ctx.setQuery(null);
          }
        },
        "inbound": async function (query, qid) {
          $ctx.setQuery(Object.assign({}, query, { "qid": qid }));
          try {
            return await inbound2($ctx, query.args);
          } finally {
            $ctx.setQuery(null);
          }
        },
        "wellKnown_Rx": async function (query, qid) {
          $ctx.setQuery(Object.assign({}, query, { "qid": qid }));
          try {
            return await wellKnown_Rx2($ctx, query.args);
          } finally {
            $ctx.setQuery(null);
          }
        },
        "autoAcceptConfig": async function (query, qid) {
          $ctx.setQuery(Object.assign({}, query, { "qid": qid }));
          try {
            return await autoAcceptConfig2($ctx, query.args);
          } finally {
            $ctx.setQuery(null);
          }
        },
