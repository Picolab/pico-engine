 ...
 */
const generateDID = krl.Function(
  ["isInvite"],
  async function (isInvite = false): Promise<DIDDoc> {
    ...
    const endpoint = `${this.module("meta")!["host"](this)}/sky/event/${
      channel.id
    }/none/dido/didcommv2_message`;
    ...
    const encryptionPublicKeyMultiCodec = new Uint8Array(
      Buffer.from(sodium.from_base64(encryptionKeyPair.publicKey.x)).length + 2
    );
    encryptionPublicKeyMultiCodec.set([0xec, 0x01]);
    encryptionPublicKeyMultiCodec.set(
      Buffer.from(sodium.from_base64(encryptionKeyPair.publicKey.x)),
      2
    );
    const base58EncryptionPublicKey = bs58.encode(
      Buffer.from(encryptionPublicKeyMultiCodec)
    );
    const signingPublicKeyMultiCodec = new Uint8Array(
      Buffer.from(sodium.from_base64(signingKeyPair.publicKey.x)).length + 2
    );
    signingPublicKeyMultiCodec.set([0xed, 0x01]);
    signingPublicKeyMultiCodec.set(
      Buffer.from(sodium.from_base64(signingKeyPair.publicKey.x)),
      2
    );
    const base58SigningPublicKey = bs58.encode(
      Buffer.from(signingPublicKeyMultiCodec)
    );
 ...
 */
const updateDID = krl.Function(
  ["did", "newDoc"],
  async function (did: string, newDoc: DIDDoc) {
    const docs = await this.rsCtx.getEnt("didDocs");
    if (docs) {
      docs[did] = newDoc;
      return true;
 ...
 */
const rotateDID = krl.Function(
  ["old_did"],
  async function (old_did: string): Promise<DIDDoc> {
 ...
 */
const rotateInviteDID = krl.Function(
  ["my_did", "their_did"],
  async function (my_did: string, their_did: string) {
    if (this.getEvent() && this.getEvent()?.eci) {
      const channels = this.rsCtx
        .pico()
        .channels.filter((c) => c.id === this.getEvent()?.eci);
      if (channels.length > 0) {
        ...
        if (channel.tags.includes("did_invite")) {
          ...
          if (!rotationExists) {
            ...
            const new_doc = await rotateDID(this, [my_did]);
            const invites = await this.rsCtx.getEnt("invites");
            this.rsCtx.raiseEvent("dido", "invitation_accepted", {
              role: "invitee",
              label: invites[my_did]["label"],
              my_did: new_doc["id"],
              their_did: their_did,
              created_time: invites[my_did]["created_time"],
            });
            // Future Task: If invite is reusable, don't delete
            delete invites[my_did];
            await this.rsCtx.putEnt("invites", invites);
          }
        }
      }
    }
 ...
 * { their_did: my_did }
 */
const mapDid = krl.Function(
  ["their_did", "my_did"],
  async function (their_did: string, my_did: string) {
 ...
 */
const updateDidMap = krl.Function(
  ["old_did", "new_did"],
  async function (old_did: string, new_did: string) {
...
const JWKFromMultibase = function (multibase: string, crv: string) {
  ...
  const key = sodium
    .to_base64(
      Buffer.from(multi_decoded.slice(2)),
      sodium.base64_variants.URLSAFE
    )
    .replace("=", "");
    ...
    parts.forEach((part) => {
      if (part[0] === "E") {
        ...
        const service = JSON.parse(
          Buffer.from(part.substring(1), "base64").toString("utf8")
        );
        services.push({
...
const unpack = krl.Function(["message"], async function (message: any) {
  try {
    const result: [Message, UnpackMetadata] = (await Message.unpack(
      JSON.stringify(message),
      new PicoDIDResolver(await this.rsCtx.getEnt("didDocs")),
      new PicoSecretsResolver(await this.rsCtx.getEnt("didSecrets")),
      {}
    )) as [Message, UnpackMetadata];
    return result;
  } catch (error) {
    this.log.error("There was an error unpacking a message: ", {
      message: message,
      error: error,
    });
    return null;
...
const pack = krl.Function(
  ["message", "_from", "to"],
  async function (message: IMessage, _from: string, to: string) {
    try {
      const _message: Message = new Message(message);
      const [enc_msg, packed_meta]: [string, PackEncryptedMetadata] =
        (await _message.pack_encrypted(
          to,
          _from,
          _from,
          new PicoDIDResolver(await this.rsCtx.getEnt("didDocs")),
          new PicoSecretsResolver(await this.rsCtx.getEnt("didSecrets")),
          { forward: false }
        )) as [string, PackEncryptedMetadata];
      return JSON.parse(enc_msg);
    } catch (error) {
      this.log.error("There was an error packing a message: ", {
        message: message,
        from: _from,
        to: to,
        error: error,
 ...
 */
const addRoute = krl.Function(
  ["type", "domain", "rule"],
  async function (type: string, domain: string, rule: string) {
      ...
      if (unpacked["type"] === "https://picolabs.io/event/1.0/event") {
        ...
        try {
          ...
          return await this.krl.assertFunction(
            this.module(unpacked["body"]["rid"])![unpacked["body"]["name"]]
          )(this, unpacked["body"]["args"]);
        } catch (error) {
        ...
        this.rsCtx.raiseEvent(
          routes[unpacked.type]["domain"],
          routes[unpacked.type]["name"],
          { message: unpacked, metadata: unpack_meta }
        );
      }
    } else {
      this.log.error("Unknown route for message: ", {
        message: unpacked,
        metadata: unpack_meta,
      });
    }
  } catch (error) {
    this.log.error("Error handling route for message: ", {
      message: unpacked,
      metadata: unpack_meta,
      error: error,
    });
 ...
 */
const send = krl.Function(
  ["did", "message"],
  async function (did: string, message: any) {
    try {
      const docs = await this.rsCtx.getEnt("didDocs");
      ...
      const eci = this.rsCtx
        .pico()
        .channels.filter((c) => c.tags.indexOf(normalizeDID(_from)) >= 0)[0][
        "id"
      ];
      ...
      this.log.error("Error sending did message: ", {
        error: error,
        did: did,
        message: message,
      });
 ...
 */
const prepareQuery = krl.Function(
  ["did", "query"],
  async function (did: string, query: any) {
    await this.useModule("io.picolabs.did-o", "didx");
    return await this.krl.assertFunction(this.module("didx")!["sendQuery"])(
      this,
      [did, query]
    );
  }
 ...
 */
const sendQuery = krl.Function(
  ["did", "message"],
  async function (did: string, message: any) {
    try {
      const docs: any = await this.rsCtx.getEnt("didDocs");
      const endpoint: string =
        docs[did]["service"][0]["serviceEndpoint"]["uri"];
      const _from: string = message["from"];
      const packed_message = await pack(this, [message, _from, did]);
      ...
      this.log.error("Error sending did query: ", {
        error: error,
        did: did,
        message: message,
      });
 ...
 */
const generateMessage = krl.Function(
  ["messageOptions"],
  async function (messageOptions: {
    ...
    if (messageOptions.expires_time) {
      message.expires_time =
        Math.floor(Date.now() / 1000) + messageOptions.expires_time;
    }
      ...
      try {
        message.from_prior =
          (
            await new FromPrior(
              (
                await this.rsCtx.getEnt("pendingRotations")
              )[messageOptions.from] as IFromPrior
            ).pack(
              null,
              new PicoDIDResolver(await this.rsCtx.getEnt("didDocs")),
              new PicoSecretsResolver(await this.rsCtx.getEnt("didSecrets"))
    ...
    if (
      ...
      let invites = await this.rsCtx.getEnt("invites");
      if (!invites) {
        invites = {};
      }
      invites[messageOptions.from] = message;
      await this.rsCtx.putEnt("invites", invites);
    }
    return message;
  }
 ...
 */
const addLabelsToChannel = krl.Function(
  ["eci", "labels"],
  async function (eci: string, labels: any) {
...
const deleteDIDChannel = krl.Function(["did"], async function (did: string) {
  await this.rsCtx
    .pico()
    .channels.filter((c) => c.tags.indexOf(normalizeDID(did)) >= 0)
    .forEach(async (c) => await this.rsCtx.delChannel(c.id));
});
