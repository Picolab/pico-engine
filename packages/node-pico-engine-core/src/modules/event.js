...
var aggregateWrap = function(ctx, value_pairs, fn, callback){
    λ.each(value_pairs, function(pair, next){
        var name = pair[0];
        var value = pair[1] === void 0
            ? null//leveldb doesnt support undefined
            : pair[1];
        ctx.db.updateAggregatorVar(ctx.pico_id, ctx.rule, name, function(val){
...
var fns = {
    ...
    aggregateEvent: cocb.toYieldable(function(ctx, args, callback){
        ...
        if(_.has(aggregators, aggregator)){
            aggregateWrap(ctx, value_pairs, aggregators[aggregator], callback);
            return;
        }
